...
def g1_body_from_30_to_29(body_pos_30: torch.Tensor) -> torch.Tensor:
    """
    将形状 (N, 30, 3) 的关节坐标转换为形状 (N, 29, 3)。
    多出来的手指等关节在输出中将填充为 (0, 0, 0)。

    参数:
    -------
        body_pos_30 : torch.Tensor
            大小为 (N, 30, 3) 的关节坐标，N 为批量大小

    返回:
    -------
        body_pos_29 : torch.Tensor
            大小为 (N, 29, 3) 的关节坐标
    """

    # 构建一个大小为 29 的整型索引张量，用于说明：
    # “29-link 的每个关节，对应到 14-link 中的哪一个下标？”
    # 若对应不到（例如手指关节），则为 -1。
    idx_map_29_list = [-1] * 29
    # 直接在列表中指定 14->29 的映射：
    # ---------------------------------------------------------------------
    idx_map_29_list[0]  = 0   # pelvis
    idx_map_29_list[1]  = 17  # left_leg
    idx_map_29_list[2]  = 18
    idx_map_29_list[3]  = 19
    idx_map_29_list[4]  = 20 
    idx_map_29_list[5]  = 21
    idx_map_29_list[6]  = 22
    idx_map_29_list[7]  = 23  # right_leg
    idx_map_29_list[8]  = 24    
    idx_map_29_list[9]  = 25
    idx_map_29_list[10] = 26
    idx_map_29_list[11] = 27
    idx_map_29_list[12] = 28
    idx_map_29_list[13] = 1  # waist yaw
    idx_map_29_list[14] = 14 # head
    idx_map_29_list[15] = 15 
    idx_map_29_list[16] = 16
    idx_map_29_list[17] = 2   # left_arm
    idx_map_29_list[18] = 3
    idx_map_29_list[19] = 4
    idx_map_29_list[20] = 5
    idx_map_29_list[21] = 6   # right_arm
    idx_map_29_list[22] = 7
    idx_map_29_list[23] = 8
    idx_map_29_list[24] = 9
    idx_map_29_list[25] = 10
    idx_map_29_list[26] = 11
    idx_map_29_list[27] = 12
    idx_map_29_list[28] = 13

    # 其余下标(手指相关)依旧保持 -1

    # 转换成 PyTorch 张量，放到和输入相同的 device 上
    idx_map_29 = torch.tensor(idx_map_29_list, 
                              dtype=torch.long, 
                              device=body_pos_30.device)

    # 创建输出张量，大小 (N, 30, 3)，默认填零
    N = body_pos_30.shape[0]
    body_pos_29 = torch.zeros((N, 29, 3), 
                              dtype=body_pos_30.dtype, 
                              device=body_pos_30.device)

    # 构建一个布尔掩码，筛选出 idx_map_52 >= 0 的关节
    valid_mask = (idx_map_29 >= 0)

    # 对有效关节通过高级索引直接复制，无需对 N 进行循环
    body_pos_29[:, valid_mask, :] = body_pos_30[:, idx_map_29[valid_mask], :]

    return body_pos_29



class OrcaMimicDistill(HumanoidMimic):
    def __init__(self, cfg: OrcaMimicPrivCfg, sim_params, physics_engine, sim_device, headless):
        self.cfg = cfg
    ...
    def _reset_ref_motion(self, env_ids, motion_ids=None):
        ...
        if body_pos.shape[1] != self._ref_body_pos[env_ids].shape[1]:
            body_pos = g1_body_from_30_to_29(body_pos)
